...
class BernoulliLikelihood_yolo(FishLikelihoodBase):
    ...
    def nll(self, preds: torch.Tensor, observations: torch.Tensor) -> torch.Tensor:
        ...
        # preds[0] is a 3-D tensor: [batch_size; sum of (no. anchor sizes * no. of grid rows * no. of grid cols ); [x ,y , width, height, class, confidences]]
    ...
    def draw(self, preds: torch.Tensor) -> torch.Tensor:
        indices = torch.tensor([i for i in range(5, preds[0].shape[2])]).to(self.device)
        preds = torch.index_select(preds[0], 2, indices)
